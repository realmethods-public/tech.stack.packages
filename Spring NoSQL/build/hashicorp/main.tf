...
#set( $dockerTag = ${aib.getParam("terraform.docker-tag")} )
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker run -it -e DEFAULT_HOST_ADDRESS=${aws_instance.mongodb.public_ip} -e DEFAULT_HOST_PORT=27017 -p 8000:8000 -p 8080:8080 ${dockerOrgName}/${dockerRepo}:${dockerTag}"
    ]
  }
}
